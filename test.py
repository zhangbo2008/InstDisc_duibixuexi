...
def NN(epoch, net, lemniscate, trainloader, testloader, recompute_memory=0):
    ...
    if hasattr(trainloader.dataset, 'imgs'):
        trainLabels = torch.LongTensor([y for (p, y) in trainloader.dataset.imgs])
    else:
        trainLabels = torch.LongTensor(trainloader.dataset.train_labels)
    ...
    if recompute_memory:
        ...
        trainLabels = torch.LongTensor(temploader.dataset.train_labels)
...
def kNN(epoch, net, lemniscate, trainloader, testloader, K, sigma, recompute_memory=0):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets, indexes) in enumerate(testloader):
            ...
            dist = torch.mm(features, trainFeatures)  #=============距离就是乘法, 也就是内机.
