...
trainloader = torch.utils.data.DataLoader(trainset, batch_size=48, shuffle=True, num_workers=2)
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets, indexes) in enumerate(trainloader):
        ...
        inputs, targets, indexes = inputs.to(device), targets.to(device), indexes.to(device) # target 标签不使用, 我们这个是非监督学习.
        optimizer.zero_grad()

        features = net(inputs)
        outputs = lemniscate(features, indexes) # 核心网络.
        loss = criterion(outputs, indexes)
        # running_loss = loss.item()#注意这里必须加item,否则爆显存.
        # print(running_loss,'打印损失')
        loss.backward()
        optimizer.step()
